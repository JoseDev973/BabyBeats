...
const translations = {
  es: {
    subject: (childName: string) =>
      `El album de ${childName} esta listo!`,
    preheader: (childName: string) =>
      `Las canciones personalizadas de ${childName} estan listas para escuchar.`,
    heading: (childName: string) =>
      `El album de ${childName} esta listo!`,
    body: (totalSongs: number) =>
      `Hemos terminado de crear ${totalSongs} canciones personalizadas llenas de amor y magia musical.`,
    cta: "Escuchar ahora",
    footer:
  ...
  en: {
    subject: (childName: string) =>
      `${childName}'s album is ready!`,
    preheader: (childName: string) =>
      `${childName}'s personalized songs are ready to listen.`,
    heading: (childName: string) =>
      `${childName}'s album is ready!`,
    body: (totalSongs: number) =>
      `We've finished creating ${totalSongs} personalized songs filled with love and musical magic.`,
    cta: "Listen Now",
    footer:
...
export function getGiftReadySubject(childName: string, lang: "es" | "en") {
  return translations[lang].subject(childName);
}

export function giftReadyEmail({
  childName,
  totalSongs,
  deliveryUrl,
  lang,
}: GiftReadyEmailProps): string {
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <div style="text-align:center;margin:24px 0;">
                <span style="display:inline-block;background-color:#ede9fe;color:#6d28d9;font-size:18px;font-weight:700;padding:12px 24px;border-radius:999px;">
                  &#127926; ${totalSongs} ${lang === "es" ? "canciones" : "songs"}
                </span>
              </div>
